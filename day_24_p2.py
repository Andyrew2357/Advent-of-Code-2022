...
SEEN={}
# row, col, time, checkpoint A, checkpoint B
...
while Q:
    ...
    r,c,t,checkA,checkB=Q.popleft()
    tf=t%Rt

    if (r,c,tf,checkA,checkB) in SEEN and t>=SEEN[(r,c,tf,checkA,checkB)]: continue
    SEEN[(r,c,tf,checkA,checkB)]=t

    # if you can finish, do so
    t+=1
    if r==M-2 and c==N-2 and checkA and checkB: ans=min(ans,t)
    ...
    for dr,dc in adj:
        ...
        if nr>M-2 and (not (nr,nc)==(M-1,N-2)): continue
        if nc<1 or nc>N-2: continue 
        ...
        if (nr,nc)==(M-1,N-2) and (not checkA) and (not checkB): 
            Q.append((M-1,N-2,t,True,False))
            continue

        if (nr,nc)==(0,1) and checkA:
            Q.append((0,1,t,True,True))
            continue

        Q.append((nr,nc,t,checkA,checkB))
